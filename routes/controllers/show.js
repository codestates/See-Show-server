...
module.exports = {
  ...
  updateDB: async () => {
    ...
    await axios({
      method: "get",
      url: `http://www.culture.go.kr/openapi/rest/publicperformancedisplays/period?serviceKey=${
        process.env.SHOW_API_KEY
      }&sortStdr=3&from=${today}&to=${afterSixMonth(
        today
      )}&cPage=1&rows=100&gpsxfrom=125.590&gpsyfrom=34.344&gpsxto=130.138&gpsyto=38.290`,
      responseType: "json",
    }).then(async (res) => {
      const convertData = convert.xml2js(res.data, {
        compact: true,
        spaces: 4,
      });
  ...
  detailInfo: async (req, res) => {
    ...
    await axios({
      ...
      const convertData = convert.xml2js(data.data, {
        compact: true,
        spaces: 4,
      });
      const {
